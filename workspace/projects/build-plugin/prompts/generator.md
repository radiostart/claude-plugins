...
| 신선도 힌트 | `pilot/tools/doctor/integrity.py` (인용 헬퍼 공용 모듈 추출) · `pilot/tools/orchestrate-load.py` (힌트) · `drift-protocol.md` · `GUIDE.md`·`state-schema.md` (F-E 문서 정정) | 기존 `check_context_citations_stale` 의 mtime 판정을 로드 시점에 노출. 헬퍼 재사용·`freshness.py` 신설 금지·doctor 출력 불변 (#28) |
