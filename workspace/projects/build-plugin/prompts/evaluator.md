...
- [x] #20 integrity.py ~1,060줄 (마이그레이션 삭제·lint 4종 이관·OH 축소) + 이관 3종 스크립트 삭제가 호출처 문서 대체와 동일 커밋 + schema.py 유지 · validate.yml CI 신설 + 이관 부적합 6종 (plan-validate·regen-verify·auto_pilot·confluence·slack-notify·docs_build) 무변경 + 연동 테스트 정리 후 pytest 전체 통과 + tools/ 30%+ 감축 + 파이프라인 1사이클 실완주. **dogfooding 게이트 2026-07-26 해소 — 전건 통과.**
  ...
  - **[통과] dogfooding 게이트 — "파이프라인 1사이클 실완주"** (2026-07-26, #23 사이클로 판정. 5단계 절차 ①머지→②배포→③설치본 갱신→④세션 재시작→⑤실경로 재측정 완주):
    - **로드 경로 전환 확인** — planner·generator·evaluator 3구간 모두 base directory `~/.claude/plugins/cache/radiostart-plugins/pilot/**0.10.0**` (구 0.4.0 아님). evaluator 구간의 step 1 은 `python3 ~/.claude/plugins/cache/radiostart-plugins/pilot/0.10.0/tools/orchestrate-load.py --phase evaluator` 로 실행됨.
    - **판정 (b) — orchestrate-load JSON 에 도메인 진입 파일 실재: 충족.** planner 구간(2026-07-25, `files_to_read` 7건 전건 실재)에 더해 **evaluator 구간에서 독립 재현**: `error: null` · `domain: "pilot"` · `files_to_read` 에 `${CLAUDE_PLUGIN_ROOT}/skills/context/shared/wrapper-protocol.md`(D2 배선) + `workspace/context/pilot/index.md` 실재 (전건 Read 성공) · hints = `MANIFEST 도메인 진입 파일 로드: context/pilot/index.md` — `"도메인 진입 파일이 MANIFEST 에 등록되지 않음"` 힌트 **부재**. 0.4.0 에서 관측되던 MANIFEST 파서 오탐이 실경로에서 소멸했다 = #20 스텝 6 변경분 반영 증거.
    - **판정 (a) — 삭제 스크립트 4종 호출 시도 0건: 충족.** planner 0건 · generator 0건 · **evaluator 구간 0건** (본 세션 실행 명령은 orchestrate-load.py · doctor.py · unittest · git/grep 뿐). 구조적 보강 증거 — 캐시 0.10.0 트리에 `diagnose.py`·`memory-hint.py`·`init_detect.py`·`verify-report-lint.py` **부재** (`ls` 4건 모두 No such file, `tools/` 는 11 엔트리) + 트리 전체 grep 결과 잔존 언급은 **provenance 주석·릴리스 노트 3건뿐** (`auto_pilot.py:82` · `agents/pilot-evaluator.md:54` · `docs/index.md:13`) 으로 **호출 지시 0건**. #21 시점 (a) 가 `skip` 이던 사유(구버전 캐시라 증거력 0)가 해소됐다.
...
- [x] #23 doctor 파서 오탐 2건 해소 — (A) `check_conventions_paths` 가 config 표 플레이스홀더를 실선언으로 오탐 (B) `count_real_features` 가 `.plan.critic.md` 를 feature 로 계수 (24 → 29 오계산 → 불필요한 `--regen-agents` 권고). doctor WARN 4 → 1 (남는 1건 = `plugin_version` 정상 감지). 정상 탐지 유지 (실선언 후 파일 부재 / 실제 spec 증가) + 기존 doctor 테스트 무손 + (B) 회귀 테스트 신규 + `integrity.py:517` analyzed 판정 불변. (검증 2026-07-26 evaluator 독립 재실측: **G1** `python3 pilot/tools/doctor.py workspace` = `10 PASS · 1 WARN · 0 ERROR` exit 0 — WARN 은 `plugin_version` 1건뿐, conventions INFO 2줄, `analyzed=True, features=24`. `workspace/context/config.md` 무변경(`git status` 빈 출력) = **파서 단독 효과** 증명 · **G2** `test_doctor_conventions.py` 8 tests OK (기존 5 케이스 diff 상 `-` 라인 0 = 무손) + `test_doctor_features_count.py` 5 tests OK · **G3** `python3 -m unittest discover -s pilot/tests/tools` **292 tests OK exit 0** (#20 baseline 284 + 신규 8) · **G4** `.agent-state.yml` diff 0, `last_analyzed_features: 24` 유지 · **정상 탐지 3종 독립 재현** (workspace 사본에 주입): (a) 실선언 `` `context/conventions.md` `` + 파일 부재 → WARN 발화 (b) spec 2건 추가 → `drift: features 24 → 26 (증가 2)` WARN 발화 (c) 파생 산출물만 남김 → `analyzed=true 이지만 features/ 에 실제 파일 없음` WARN 발화 · **승인 규칙 3건 일치**: (A) 구조 기반 4단 판정 (`integrity.py:874-893`, 마커 → 코드 스팬 단독 → 공백 없는 평문 → 그 외 ambiguous), (A) `--fix` 미확장 (`run_auto_fixes` diff 0), (B) `is_feature_spec_file` 다중 확장자 규칙을 `_common.py:197` 1곳에 고정 + `count_real_features` 하드코딩 `.plan.md` 제거 · `integrity.py:517` 호출부 diff 0 (call site 실측 1곳) · `doctor.py --schema` `6 PASS` exit 0)
- [x] #24 `tools/context-search.py` 신규 — 질의 3형식(`select:`·키워드·`+필수어`)·점수표(10/8/6/5/4/2)·top-N(기본 5·최대 20)·json/md 출력·read_hint·0건 상태 안내·traversal 거부. 결정적(동일 입력→동일 출력)·읽기 전용·표준 라이브러리만·1,000 섹션 300ms 이내. orchestrate-load 힌트 1줄 + wrapper-protocol §6 권장 문구 + scope-exploration Explore 계약 + confluence.py 랭커 공유. 골든 질의 3개 `hit@3` 통과. 래퍼 필수 step 추가 0·지시 문서 순증 ≤30줄. — evaluator 검증 완료 (2026-09-04, 독립 재실측): `python3 -m unittest discover -s pilot/tests/tools` 386 OK exit 0 · fixture 골든 4/4 + 라이브 `--scope pilot` 4질의 top-1 정답(22/22/66/14) · 동일 질의 2회 json diff 0(4질의+무-scope) · 1,000섹션(인용 6/섹션) `search()` 83~95ms·CLI 벽시계 130~200ms · traversal 6케이스 exit 2+stderr · 심링크 탈출 제외+INFO · 도구 실행 전후 `git status --short workspace/context` 변화 0 · `git diff --stat pilot/agents/` 빈 결과 · 지시 문서 numstat 순증 +2 · import 는 stdlib 만 · 저장소판 orchestrate-load 힌트 `[검색] … --scope pilot` 1줄 실재. 알려진 nit(비차단): `context-search.py:527` C8 조사 안내 예시가 `{t0}을`→`{t0}` 로 렌더돼 토큰 `섹션을` 에서 `섹션을을`→`섹션을` 출력 — 전달사항 등록
- [ ] #25 `tools/freshness.py` 신규 — 인용 파싱·기준 시각(`learned_at`>git>mtime)·changed/total/missing 카운트 → orchestrate-load `[신선도]` 힌트(≤1일·변경 0 이면 생략) + doctor WARN(≥30% 또는 missing ≥1)/INFO. 지연 ≤200ms. 실사례: 현 `workspace/context/pilot/` 로드 시 `미존재 ≥1` 발화. F-E: `GUIDE.md`·`state-schema.md` 의 analyzed 재로드 서술 정정 + `build_load_plan` 코드 diff 0.
- [ ] #26 `/pilot:learn` 산출 frontmatter 5 키 + orchestrate-load `context_manifest`(30줄 스캔·200 캡·본문 미로드·진입 파일은 `files_to_read` 유지) + doctor WARN 3종(description 부재·150자 초과·색인 200줄/25KB)·INFO 2종 + `--fix` 마이그레이션 제안(승인 시에만 기입). MANIFEST.md 는 frontmatter 없음. #24 `hit@3` 저하 없음.
- [ ] #27 선행 실측 기록(feature § 실측 기록) 후 C1/C2 확정 → C1: `/pilot:learn` Phase 5 가 `.claude/rules/pilot-{domain}.md`(paths=sources·관리 마커·포인터 ≤8줄) 생성, 마커 없는 파일 무변경 / C2: PostToolUse 훅 additionalContext(턴·도메인 1회·500자·100ms). doctor stale·경로 부재·paths↔sources 검증. 지식 본문 복사 0.
...
- [x] #24 코퍼스 기본 = 지식 루트만(`--include` 없이 features/·docs/ 미포함) · 코드블록 내 `#` 헤딩 미인식 · `--scope` 미등록 도메인 → 전체 fallback + INFO · 인용 경로 존재 검증 안 함(#25 소관) · confluence.py 섹션 분할 규칙 기존 유지 — evaluator 검증 완료 (2026-09-04): `collect_files` 기본 경로 = `{workspace}/context` 만(`--include` 없으면 include_candidates 빈 집합) · `--include features/ docs/` 시 STATE.md 진행중 `build-plugin` 으로 해석돼 `projects/build-plugin/features/*` 반환 확인 · 펜스(들여쓴 펜스 포함) 안 `#` 미인식을 CLI `select:` 로 확인 · `--scope nosuchdomain` → 후보 9건 전체 + INFO 1줄 exit 0 · 인용 경로 존재 검증 코드 없음 · confluence `search_docs` 분할 정규식 `\n(?=#{1,2} )`·`(서문)` 기존 유지
- [ ] #25 자동 수정·자동 재학습 트리거 0 · git 부재/실패 시 mtime fallback + `(mtime 기준)` 표기 · 인용 500 초과 표본 표기 · 기존 "context mtime > analyzed_at" 검사와 별개 축 공존(중복 WARN 없음)
- [ ] #26 description 자동 기입 0(제안·승인만) · MANIFEST.md frontmatter 없음 · scope/·rules/ 커스텀 layer 미강제 · frontmatter 파싱 실패 시 표기 + WARN, abort 없음 · 회귀 픽스처 `learn/expected/` 재캡처
- [ ] #27 실측 전 구현 착수 0 · 규칙 파일 본문 = 포인터만(지식 본문 복사 0) · 관리 마커 없는 파일 무변경 + INFO · 규칙 파일 위치 저장소 루트 `.claude/rules/` 1곳 · 차단 훅(scope-guard)과 컨텍스트 훅 분리
...
- [x] `pilot/tests/tools/test_doctor_conventions.py` (#23) — 8/8 PASS (기존 5 + 신규 3: placeholder-not-declared / plain-text-still-declared / dash-marker-silent). 기존 5 케이스 무변경
- [x] `pilot/tests/tools/test_doctor_features_count.py` (#23 신규) — 5/5 PASS (mixed spec+derived → 3 / derived-only → 0 / 미래 접미사 `.plan.review.md` 자동 제외 / 하위 디렉터리·비 md 무시 / dotted-stem spec 미카운트 경계 고정)
- [x] `pilot/tests/tools/test_context_search.py` (#24 신규) — 토큰화·점수·필수어·라인 범위·코드블록·select·0건·traversal — evaluator 검증 완료 (2026-09-04): 단독 86 OK · discover 포함 386 OK. 미커버 1건: C8 조사 안내 문자열(전달사항 참조)
- [ ] `pilot/tests/tools/test_freshness.py` (#25 신규) — 변경/미변경/삭제 픽스처 + git/mtime fallback + 임계 경계(30%·missing 1)
- [ ] `pilot/tests/tools/test_orchestrate_load.py` 확장 (#25 힌트 · #26 `context_manifest`) + doctor 테스트 확장 (#26 캡 3종 · #27 규칙 파일 3종)
- [ ] (C2 채택 시) `pilot/tests/tools/test_context_pointer.py` (#27 신규) — 매칭·비매칭·다중 도메인·500자 상한
