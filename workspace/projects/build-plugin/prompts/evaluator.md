...
- [ ] #24 `pilot-update.sh` — (A) `CACHE_DIR` stale 경로(`claude-plugins` → `radiostart-plugins`) 해소 (B) 도구 존치 여부 결정 반영 (C) README·getting-started·릴리스 노트의 잘못된 업그레이드 안내 정정. 게이트: stale 경로 문자열 0건 + 안내가 실제 동작과 일치 + 전역 설치본 직접 조작 0. **미체크 사유 = (C) 4곳 중 릴리스 노트 1곳 미정정 (나머지 전건 통과).**
