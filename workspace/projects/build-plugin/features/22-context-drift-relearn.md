...
- **트리거**: `workspace/context/pilot/` 산출물이 삭제된 스크립트를 현행 구현으로 서술 중. #21 planner 실측 → critic C6 로 1건 → **3건** 정정된 인벤토리가 스코프 근거다.
- **기대결과**: 아래 3건이 현행 구현과 일치하도록 재학습으로 해소된다.
...
- 라인 번호는 재학습 시점에 이동한다. 위 표의 `:NN` 은 등록 시점 좌표이며, 검증은 **문자열 기준**(`memory-hint`·`init_detect`·`diagnose.py`)으로 한다.
...
### (d) 비즈니스 결정 영역
- [x] `/pilot:learn` 재실행이 3건을 전부 덮지 못할 경우 — drift-protocol § B 로 전환해 승인 하 직접 정정할 것인가, 아니면 learn 스킬 자체의 커버리지 결함으로 보고 별도 처리할 것인가 → **분기 불성립 (2026-08-01)**: 재실행이 6 파일을 전부 재생성해 3건 모두 문자열 기준 0건. drift-protocol § B 전환 불요
