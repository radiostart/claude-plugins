...
- **기대결과**:
  ...
  - 정정 (감사 축 2 § B 판정대로): B-1~B-9 모순 드리프트 — 단 INDEX.md 삭제로 자동 해소되는 건 (B-1·B-2 일부·B-6·A-12) 은 삭제로 갈음, 잔존 파일 쪽 (analyze/SKILL.md:202 B-2, pr/SKILL.md:49 B-4, GUIDE.md B-3·B-5, code-review-init B-7, tdd/SKILL.md:13 B-8, INFO 문구 B-9) 은 정본 기준으로 문구 정정
...
- v0.1.0-baseline 삭제 시 자동 테스트가 참조하는 서브디렉터리 (`external-domain`·`transaction-contracts`·`config`·`migration`·`open-questions`·`verify-reports`) 를 오삭제하면 pytest 가 깨진다 — 삭제 전 테스트 참조 재확인 필수
