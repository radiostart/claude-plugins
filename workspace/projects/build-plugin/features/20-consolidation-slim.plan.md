...
> 본 게이트의 판정 근거는 **저장소 사본(`pilot/tools/…`) 직접 실행 결과로 한정**한다. wrapper 가 실제로 로드하는 것은 설치 캐시 `~/.claude/plugins/cache/radiostart-plugins/pilot/` 의 **0.4.0** 이고 저장소는 `plugin.json` 기준 **0.9.0** 이라, 사이클 완주가 #20 변경분의 **실경로 통과를 의미하지 않는다**. **(a) 안(캐시를 저장소 사본으로 로컬 교체 후 실경로 검증) 기각 사유 — 정정본 (2026-07-25 evaluator, critic C9 반영. 구 문안 "마켓플레이스가 GitHub 클론이라 머지·배포 선행 필요" 는 사실과 다름):** 설치 위치는 `~/.claude/plugins/installed_plugins.json` 의 로컬 디렉터리 + 로컬 레지스트리라 캐시 교체 경로 자체는 **기술적으로 존재**한다. 기각한 실제 이유는 ① 사용자 전역 플러그인 설치본을 검증 목적으로 변조하는 것이 부적절하고 ② 세션 중 플러그인 리로드가 불가해 교체하더라도 wrapper 실경로 재현이 성립하지 않기 때문이다.
> **한계 명시** — 캐시 0.4.0 은 애초에 삭제 대상 4파일을 참조하지 않는 구버전이므로, 무증상 통과를 하위호환 신호로도 해석할 수 없다.
> **후속 확인 필요** — 본 항목은 **미완**으로 남는다. "배포만 되면 자동 해소" 가 아니다. 재확인 절차 5단계: ① PR 머지 → ② 배포 → ③ `pilot/tools/pilot-update.sh` 실행 → ④ **세션 재시작** (세션 중 플러그인 리로드 불가) → ⑤ 실경로(설치 캐시 경유 wrapper 세션)에서 아래 (b) 기대값 재측정.
