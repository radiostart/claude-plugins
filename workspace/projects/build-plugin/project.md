...
- [ ] **릴리스 시 pilot-v0.13.0 태그 동기 필요** — `doctor --schema` 가 `version(0.13.0) ≠ 최신 태그(pilot-v0.10.1)` WARN 발화 중 (릴리스 전 정상 상태). PR 머지 → 릴리스 시점에 CHANGELOG 갱신 + `pilot-v0.13.0` 태그 생성으로 해소 — v0.11.0 도 동일 절차였음 (from #26)
