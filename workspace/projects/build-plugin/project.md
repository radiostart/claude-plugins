...
- [ ] **#28 착수 전 확인 — 원격 v0.16.0 의 doctor `인용 drift 검사`(인용 소스 변경 감지 WARN) 가 #28 spec 의 doctor 파트와 겹친다.** #28 planner 는 기존 검사와의 통합(신선도 힌트 = 로더 시점 · 기존 = 점검 시점)과 중복 WARN 방지를 먼저 결정할 것. 병합 직후 `context/pilot/{index,spec}.md` 가 `orchestrate-load.py` 힌트 1줄 변경으로 stale WARN 2건 — #28 실사례로 활용 가능 (from #27 머지, 2026-09-04)
