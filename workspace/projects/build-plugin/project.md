...
- [x] issue 단건 사이클 + 폴더 slug 자동 명명 (dp-skills 0.25.0/0.30.0 포팅) -> [상세](features/26-issue-cycle-slug.md) `[dp-sync]`
- [x] context-search 섹션 단위 결정적 검색 도구 -> [상세](features/27-context-search-tool.md) `[retrieval A]`
- [ ] 로드 시 신선도 힌트 + 로드 정책 문서 정합 -> [상세](features/28-load-freshness-hints.md) `[retrieval D+E]`
- [ ] 본문 frontmatter 매니페스트 -> [상세](features/29-frontmatter-manifest.md) `[retrieval B]`
- [ ] 경로 트리거 — 도메인 포인터 자동 로드 -> [상세](features/30-path-triggered-context.md) `[retrieval C · 실측 선행]`
...
- [x] `pilot/docs/reference/{agents,skills,tools}/` 는 `pilot/.gitignore:10` 로 **git 미추적** — `docs_build.py` 재생성 여부는 `git diff`·커밋 diffstat 로 증명 불가하고 **현재 파일 상태 + `--check` exit code** 로만 확인된다. 후속 evaluator 가 "docs 재생성 실행" 류 항목을 판정할 때 커밋 증거를 요구하면 오탐 — 상태 기반 증거(생성 페이지 목록 ↔ 소스 실재 목록 일치)로 대체할 것 (from #20) — 소화 (2026-09-04): #27 plan G6 이 상태 기반 증거(파일 실재·docs_build --check)로 반영
- [x] **#20 dogfooding 체크 조건** — `project.md` `## 목표` 의 #20 은 `[ ]` 가 정상 상태 (미완 누락 아님). 체크 조건 = 배포 후 **설치 캐시 실경로** 1사이클 검증. 절차 5단계: ① PR 머지 → ② 배포 → ③ `/plugin marketplace update radiostart-plugins` + `/plugin update pilot@radiostart-plugins` (#24 로 `pilot-update.sh` 삭제됨) → ④ **세션 재시작** (세션 중 플러그인 리로드 불가) → ⑤ 실경로에서 G7 기대값 재측정 (`files_to_read` 에 `wrapper-protocol.md`·`context/pilot/index.md` 실재 + 미등록 힌트 부재) (from #21) — 소화 확인 (2026-09-04 사용자 결정: 게이트 해소 선언으로 종결, 장부 정리)
...
- [x] **[#20 dogfooding 게이트 해소 — 위 `#20 dogfooding 체크 조건` 항목은 소화됨]** 2026-07-26 #23 사이클이 설치 캐시 실경로 `~/.claude/plugins/cache/radiostart-plugins/pilot/0.10.0` 에서 planner→generator→evaluator 완주. 판정 (a) 삭제 스크립트 4종 호출 0건 · (b) `files_to_read` 에 `wrapper-protocol.md`·`context/pilot/index.md` 실재 + 미등록 힌트 부재 — 3구간 전건 충족으로 `project.md` #20 목표 `[x]` 처리. 후속 사이클은 이 게이트를 다시 판정할 필요 없다 (from #23) — 소화 확인 (2026-09-04 사용자 결정: 게이트 해소 선언으로 종결, 장부 정리)
...
- [ ] **orchestrate-load placeholder leak (후속 feature 대상)** — `parse_lang_config` (`pilot/tools/orchestrate-load.py:141-172`) 가 config.md 같은 표를 파싱하며 `test_framework_hints=자유 텍스트` 같은 **플레이스홀더를 실값으로 반환** (#23 evaluator step 1 반환 JSON 에서 재실측: `config: {"test_framework_hints": "자유 텍스트"}`). 성격은 #23 (A) 와 동일하나 wrapper hints 출력이 바뀌므로 별건 — #23 이 만든 구조 기반 판정(`integrity.py:_extract_declared_path`)을 재사용해 해소 가능 (from #23). #26 은 `build_load_plan` 시그니처 (`work_mode` kwarg)·step 8 `spec_md_abs` 구조만 바꿨고 `parse_lang_tools`·`parse_lang_override` 는 diff 0 — 충돌 없음 (from #26 교차 확인) — 이월 (2026-09-04 사용자 결정: #27 context-search 범위 밖, 별건 feature 로)
- [ ] **#22 재학습은 #26 의 issue 재정의 stale 을 함께 소화해야 한다** — `workspace/context/pilot/` 의 issue 서술이 재차 stale (index.md:3 "17 개 스킬" 서술 및 lifecycle 클러스터의 issue 경량 모드 전제, spec.md 의 issue SKILL 라인 인용). #26 으로 `/pilot:issue` 가 사이클 지원 + slug 규약으로 재정의되고 preamble P1·focus·commit 거동이 바뀜 — `/pilot:learn ./pilot/skills` 재실행 시 기존 3건 (memory-hint·init_detect·diagnose.py) 과 함께 일괄 해소 (직접 Edit 금지 — drift-protocol § A. doctor 가 spec.md mtime > analyzed_at WARN 으로 기감지 중) (from #26)
- [ ] **generator plan 부재 케이스는 의도적 공백 (critic C5 합의)** — planner 없이 `@pilot-generator` 직접 호출된 issue 세션은 `issue.plan*.md` 부재로 step 2 "없으면 skip" 이 적용돼 변경 범위 게이트 기준 (`결함 함수:` 1줄) 없이 진행될 수 있다. dp HEAD (`ag-generator.md:58`) 동일 공백 — 이식 패리티로 가드 미신설 (임의 개선 금지). 실사용 중 사고 사례 발견 시 별도 feature 로 상정 (from #26)
- [ ] **릴리스 시 pilot-v0.13.0 태그 동기 필요** — `doctor --schema` 가 `version(0.13.0) ≠ 최신 태그(pilot-v0.10.1)` WARN 발화 중 (릴리스 전 정상 상태). PR 머지 → 릴리스 시점에 CHANGELOG 갱신 + `pilot-v0.13.0` 태그 생성으로 해소 — v0.11.0 도 동일 절차였음 (from #26) — **해소 완료 (2026-08-01)**: PR #14·#15·#16 이 무bump 병합돼 최종 버전은 **0.14.0** 이 됐고 (`chore(release)` PR #20 이 표기 3곳 동기), `pilot-v0.14.0` 태그 + GitHub Release 발행으로 `doctor --schema` 가 `6 PASS · 0 WARN` 이 됐다. 별도 CHANGELOG 파일은 없다 — 릴리스 노트가 그 역할
- [ ] #27 spec 검증 기준 3항 "dogfooding: 후속 feature 1건에서 planner 가 도구 1회 이상 사용 → 부분 Read" 는 #27 사이클 안에서 측정 불가 — plan § 교차 의존대로 **#28 planner** 가 `python3 pilot/tools/context-search.py "<키워드>" --scope pilot` 1회 이상 호출 + `read_hint` 대로 부분 Read 한 기록을 `25-*.plan.md` 에 남긴다. 주의: 설치 캐시(`~/.claude/plugins/cache/radiostart-plugins/pilot/0.10.0`)의 orchestrate-load 는 #27 이전 버전이라 wrapper step 1 hints 에 `[검색]` 줄이 안 나온다(evaluator 재실측) — 배포 전까지는 저장소판 경로로 직접 호출 (from #27)
- [x] `pilot/tools/context-search.py:527` 0건 안내의 C8 조사 제거 예시가 `(예: `{t0}을` → `{t0}`)` 로 렌더돼, 트리거 케이스(조사 붙은 토큰 `섹션을`)에서 `섹션을을` → `섹션을` 이 출력됨(evaluator 실측 `"zzqq 섹션을" --scope pilot`). plan 원문 고정 예시 `(예: `섹션을` → `섹션`)` 로 되돌리거나 실제 토큰만 표시할 것. 단위 테스트 미커버(`test_zero_hit_reports_token_hits_and_guidance` 는 scope 안내만 단언) — 수정 시 테스트 1건 동반. 랭킹·exit 코드·게이트 무영향, 비차단 nit (from #27) — 소화 (2026-09-04, 커밋 전 수정: 조사로 끝나는 토큰은 그 토큰, 아니면 고정 예시 `섹션을`→`섹션` 렌더 + `test_zero_hit_josa_guidance_never_doubles_particle` 추가)
- [x] **#28 착수 전 확인 — 원격 v0.16.0 의 doctor `인용 drift 검사`(인용 소스 변경 감지 WARN) 가 #28 spec 의 doctor 파트와 겹친다.** **해소 (2026-09-04 사용자 승인)**: #28 범위를 "기존 `check_context_citations_stale` 의 판정을 로드 시점에 노출" 로 축소. 기준 시각 3단 우선순위와 `freshness.py` 신설 폐기, 인용 헬퍼는 공용 모듈 추출로 1벌 유지. `learned_at` frontmatter 키는 #29 에서 삭제 (knowledge-sync 가 learn 외 writer 라 값이 갱신되지 않음). 원문: #28 planner 는 기존 검사와의 통합(신선도 힌트 = 로더 시점 · 기존 = 점검 시점)과 중복 WARN 방지를 먼저 결정할 것. 병합 직후 `context/pilot/{index,spec}.md` 가 `orchestrate-load.py` 힌트 1줄 변경으로 stale WARN 2건 — #28 실사례로 활용 가능 (from #27 머지, 2026-09-04)
