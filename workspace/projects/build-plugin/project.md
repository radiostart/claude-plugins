...
- [ ] `wrapper-protocol.md` (A-2 정본) 는 orchestrate-load `files_to_read` 에 미배선 — 각 wrapper 본문의 "Read 지시 1줄" (잔류 최소 셋 ④) 이 유일한 도달 경로 (critic C2 합의). #20 에서 orchestrate-load.py 수정 시 files_to_read 배선 추가로 이중화 검토 (from #19)
