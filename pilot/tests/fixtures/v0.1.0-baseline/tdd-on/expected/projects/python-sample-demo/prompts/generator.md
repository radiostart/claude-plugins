<!-- pilot-tdd-original-generator:start -->
<!-- pilot-tdd-original-generator:end -->
...
이 프로젝트의 관련 도메인 지식. 래퍼(`@pilot-generator`) 가 자동 로드하므로 에이전트가 재요청하지 않는다.

- `workspace/context/MANIFEST.md`
- `workspace/context/rules/{domain}.md`
- `workspace/context/scope/{domain}.md`

> `/pilot:analyze` 실행 시 `{domain}` 이 구체 도메인명으로 치환되고, 아래에 `## 핵심 서비스/모델` 섹션이 자동 주입된다.

---

<!-- [analyze-managed] -->

_(analyze 실행 후 `## 핵심 서비스/모델` 이 이 자리에 주입된다.)_
