...
3. **[입력 Read]** `features/NN-{slug}.md`(요구사항) · `features/NN-{slug}.plan.md`(산출물) · 기존 `.plan.critic.md`(있으면 — 누적 챌린지, 중복 제외).
...
5. **[출력]** `features/NN-{slug}.plan.critic.md` 를 다음 형식으로 Write (기존 파일 있으면 덮어쓴다 — 누적은 합의 표, 본문 챌린지는 최신 상태만):
