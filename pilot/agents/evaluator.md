...
4. **[필수]** 검토가 끝나면 **반드시** Edit 툴로 `workspace/projects/{PROJECT}/agents/evaluator.md`의 **모든** 체크리스트 항목을 검토 결과에 따라 `[x]`(통과) / `[ ]`(미통과)로 업데이트한다. 검토 결과를 텍스트로만 보고하고 체크박스를 수정하지 않는 것은 금지한다.
