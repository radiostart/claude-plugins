...
2-bis. Explore 를 쓰지 않는 경우 — 경로를 알면 Read · 클래스/정의 1개는 Grep/Glob · 2~3 파일이면 직접 Read. Explore·Plan 에는 `.claude/rules` 의 도메인 규칙 포인터가 로드되지 않으므로(#30 실측 기록) 규칙 확인은 결론을 받은 뒤 메인에서 한다.
