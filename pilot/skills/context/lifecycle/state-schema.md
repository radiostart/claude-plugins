...
domain: null                         # 도메인 (retail/global/wholesale/return/inventory/dms) — null 이면 analyze 전 질의 필요
...
- 값: `retail` / `global` / `wholesale` / `return` / `inventory` / `dms` / `null`
- `null` → `/pilot:analyze` 진입 시 **반드시 사용자에게 질의** 후 값 기록. 자동 추론으로 기록 금지 (후보 제시는 허용 — 사용자 확인 필수).
- 값이 있으면 analyze·doctor 가 scope/{domain}.md·rules/{domain}.md 자동 로드 대상으로 사용.
