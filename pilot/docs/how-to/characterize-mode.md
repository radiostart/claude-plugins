...
!!! info "한 줄 요약"
    기존 코드의 *현재 동작* 을 테스트로 포착하는 모드. 구현 변경 없이 (`{source_root}` 잠금) 테스트만 추가해 안전망을 먼저 확보. 리팩터는 별도 사이클로 분리.
...
- **Generator** — `{source_root}` *수정 금지*. 테스트 파일만 추가. 실제 동작을 호출해서 "현재 출력" 을 plan.md 에 다시 적는다.
