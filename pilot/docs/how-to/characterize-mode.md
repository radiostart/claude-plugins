...
핵심 차이는 **버그도 그대로 포착한다** 는 점이다. characterization 테스트의 목적은 코드를 *고치는* 게 아니라 *현 상태를 동결* 하는 것이다. 버그 수정·동작 개선은 안전망이 갖춰진 뒤 별도 사이클에서 한다 — 그래야 "내가 의도한 변경" 과 "실수로 깨뜨린 회귀" 가 구분된다.
