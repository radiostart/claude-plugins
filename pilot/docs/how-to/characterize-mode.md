...
!!! info "한 줄 요약"
    테스트 없는 기존 코드를 리팩터하기 전에 — *지금 그 코드가 실제로 하는 동작* 을 그대로 테스트로 고정하는 모드. 구현은 한 줄도 바꾸지 않고 (`{source_root}` 수정 금지) 테스트만 추가해 회귀 안전망을 먼저 친다.
...
`{source_root}` 를 건드리지 않는 것이 안전망의 신뢰성을 보장한다. 안전망을 만드는 도중 구현까지 같이 바꾸면, 테스트가 포착한 게 *원래 동작* 인지 *방금 바꾼 동작* 인지 알 수 없다. 구현에 손대지 않아야 "이 테스트들은 네가 손대기 *전* 의 코드를 기술한다" 가 보장된다. Generator 가 픽스처·헬퍼 같은 테스트 계층을 손보는 건 허용되지만, `{source_root}` 아래는 1 줄도 안 된다 — Evaluator 가 `git diff` 로 강제한다.
...
- **Generator** — `{source_root}` 는 손대지 않고 대상 코드를 실제로 실행하고, 관찰된 출력·사이드 이펙트를 테스트 assertion 으로 기록한다. 각 스텝에 `[Captured]` 증거 라인 (실행 명령·통과 시각·미수정 확인) 을 남긴다.
