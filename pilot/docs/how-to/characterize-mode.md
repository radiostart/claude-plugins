...
원칙 한 줄: **"곧 건드릴 코드인데 안전망이 없다" 가 확인되는 순간, 그 코드를 건드리기 *전* 에 시작한다.**

대표적인 시작 시점:

- 테스트 없는 코드를 **리팩터·구조 변경** 하기 직전.
- **의존성·프레임워크 업그레이드** 직전 — 조용히 동작이 바뀌는 것을 잡기 위해.
- 무테스트 코드의 **버그를 고치기** 직전 — 주변 동작을 먼저 고정해 두면, 나중에 "내가 의도한 곳만 바뀌었다" 를 테스트로 증명할 수 있다.
- 낯선 **레거시 코드를 인수** 받아 동작부터 파악해야 할 때 — characterization 테스트가 곧 *실행 가능한 명세서* 역할을 한다.
- planner·critic 단계에서 "이 변경이 무테스트 레거시를 건드린다" 는 회귀 리스크가 드러날 때 — 본 작업에 들어가기 전에 characterize 사이클을 따로 끼운다.

!!! warning "반드시 *변경 전* 에 시작"
    Characterize 는 코드를 건드리기 *전* 에 시작해야 의미가 있다. 이미 수정한 코드를 포착하면 — 포착된 건 *원래 동작* 이 아니라 *바뀐 동작* 이다. 안전망이 아니라 그저 *바뀐 상태의 사진* 이 된다.

건너뛰어도 되는 경우:

- **신규 코드를 작성** 하는 거라면 → [TDD 모드](tdd-mode.md) 가 맞다 (테스트를 먼저 쓰며 만든다).
- 대상 코드에 **이미 테스트가 충분** 하면 → 안전망이 있으니 불필요.
- 코드를 **읽기만** 하고 바꿀 계획이 없으면 → 모드 전환 자체가 불필요.

## 전제

- 활성 프로젝트가 있고, 포착 대상 코드가 이미 존재한다 (신규 작성이 아닌 *기존 동작 보존* 작업).
