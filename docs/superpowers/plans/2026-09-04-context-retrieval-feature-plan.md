...
- **기준 시각**: 파일 **mtime 단일** (2026-09-04 정정 — Open Q (d)). 대상 프로젝트에 이미 인용 stale 검사가 있으면 **그 검사와 같은 기준·같은 함수**를 쓴다. 기준이 갈리면 같은 파일에 대해 두 검사의 결과가 어긋난다.
- **인용 파싱**: 기존 검사가 있으면 그 정규식·해석 헬퍼를 재사용하고 새로 만들지 않는다. 없을 때만 신설한다 (경로 + 선택적 앵커, 같은 경로는 1회로 합침).
- **경로 해석**: 저장소 루트 기준 → 실패 시 `{{SOURCE_ROOT}}` 기준 → 실패 시 지식 파일의 디렉토리 기준. 셋 다 실패면 `missing`.
- **변경 판정**: 인용 파일 mtime 이 지식 파일 mtime 보다 뒤면 `stale`. 해석 실패는 `unresolved` 로 따로 센다.
...
- **A 연동**: A 점수표의 description 4점 활성. D 와는 데이터 의존이 없다 (D 의 기준은 mtime 단일).
...
- **(d) 신선도 기준 시각** — 프로젝트에 이미 인용 stale 검사가 있으면 그 기준을 그대로 쓴다. → **결정(2026-09-04): mtime 단일** (당초 `learned_at` > git > mtime 3단 결정을 같은 날 정정). 정정 사유: 지식 문서의 writer 가 생성기 하나가 아니면 (pilot 의 knowledge-sync 처럼 사이클마다 갱신) 생성 시각 필드는 갱신되지 않아 실제보다 오래된 값이 된다. 그리고 기존 검사와 기준이 갈리면 결과가 어긋난다. **B 의 `learned_at` 키도 함께 삭제.**
